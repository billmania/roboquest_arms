...
class SO101Follower(Robot):
    """Control the SO101 follower arm.
    ...
    def __init__(self, config: SO101FollowerConfig):
        """Create the SO101Follower class."""
    ...
    def observation_features(self) -> dict[str, type | tuple]:
        """Return the observation features."""
        return {**self._motors_ft, **self._cameras_ft}

    @cached_property
    def action_features(self) -> dict[str, type]:
        """Return the action features."""
        return self._motors_ft

    @property
    def is_connected(self) -> bool:
        """Return True when the arm is connected."""
        return (
            self.bus.is_connected and
            all(
    ...
    def is_calibrated(self) -> bool:
        """Return True when the arm is calibrated."""
        return self.bus.is_calibrated

    def calibrate(self) -> None:
        """Calibrate the motors.

        If the calibration file doesn't exist, then run the manual
        motor calibration process. Otherwise, apply the values from
        ...
        if not self.calibration:
            ...
            input(
                f'Move {self}'
                ' to the middle of its range of motion and press ENTER....'
            )
    ...
    def configure(self) -> None:
        ...
        with self.bus.torque_disabled():
            ...
            for motor in self.bus.motors:
                ...
                # lower value to avoid shakiness (Default is 32)
    ...
    def setup_motors(self) -> None:
        """Configure the motors in the arm."""
        for motor in reversed(self.bus.motors):
            input(
                'Connect the controller board to the'
                f"{motor}' motor only and press enter."
            )
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        The relative action magnitude may be clipped depending on
        the configuration parameter `max_relative_target`. In this
        case, the action sent differs from original action. Thus,
        this function always returns the action actually sent.
        ...
        goal_pos = {
            ...
            for key, val
            in action.items() if key.endswith('.pos')
        }
